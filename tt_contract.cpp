...
//#pragma HLS UNROLL
//                	for (int i_in_o2 = 0; i_in_o2 < PARALLEL_DEGREE2; i_in_o2++) {
//#pragma HLS UNROLL
...
#pragma HLS UNROLL
				array[(out_offset + ind_out) / PARALLEL_DEGREE * PARALLEL_DEGREE + i_in_o]
					  = res[i_w_2 / PARALLEL_DEGREE2][(i_w_2 % PARALLEL_DEGREE2) * PARALLEL_DEGREE + i_in_o];
			}
	    }
	}
...
void tensor_cont_last_store(
	...
	for (int i_w_1 = 0; i_w_1 < array_weight_size_1; i_w_1++) {
		for (int i_in_0o = 0; i_in_0o < PARALLEL_DEGREE2; i_in_0o++) {
			array[(out_offset + (i_in_0 + i_in_0o) * array_weight_size_1) / PARALLEL_DEGREE * PARALLEL_DEGREE
				    + i_w_1]
				  = res[i_in_0o][i_w_1];
			res[i_in_0o][i_w_1] = 0;
		}
